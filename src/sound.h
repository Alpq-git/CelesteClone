...
static constexpr int MAX_CURRENT_SOUNDS = 16;
static constexpr int SOUND_BUFFER_SIZE = MB(128);
...
struct SoundState
{
    ...
    // Allocated sounds
    Array<Sound, MAX_CURRENT_SOUNDS> allocatedSounds;

    // Used by the platform to determine when to start and stop sounds
    Array<Sound, MAX_CURRENT_SOUNDS> playingSounds;
};
...
void play_sound(char* soundName, SoundOptions options = 0)
{
    ...
    // option are supplied, eat at least SOUND_OPTIONS_START
    // should be used
    ...
    sprintf_s(sound.file, ArraySize(sound.file), "assests/sounds/%s.wav",soundName);
    ...
    if(!wavFile)
    {
        if(wavFile->header.dataChunkSize > SOUND_BUFFER_SIZE - soundState->bytesUsed)
        {
            SM_ASSERT(0, "Exausted Sounds Buffer!\nCapacity:\t%d\nBytes Used:\t%d\nSound Path:\t%s\nSound Size:\t%d", 
                        SOUND_BUFFER_SIZE, soundState->bytesUsed, sound.file, wavFile->header.dataChunkSize);
            return;
        }
