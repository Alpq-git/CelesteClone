...
static xAudioVoice voiceArr[MAX_CURRENT_SOUNDS];
...
LRESULT CALLBACK windows_window_callback(HWND window, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(msg)
    {
        ...
        case WM_KEYDOWN:
        case WM_KEYUP:
        case WM_SYSKEYDOWN:         //System key is that button pushed with ALT
        case WM_SYSKEYUP:
        {
...
bool platform_create_window(int width, int height, const char* title)
{
    ...
    wc.lpszClassName = "CelesteWindowClass";
    ...
    // Create a Handle to fake OpenGL Rendering Context
    ...
    {
        SM_ASSERT(false, "Failed to open OpenGL funtions");
        return false;
    }

    //Clean up to take stuff
    ...
    window = CreateWindowExA(0,
                                "CelesteWindowClass", //This references lpszClassName from wc
...
{
    HMODULE result = LoadLibrary(dll);
...
bool platform_init_audio()
{
    ...
    hr = xaudio->CreateMasteringVoice(&master_voice);
    if(FAILED(hr)) { return false;}
    ...
    for(int voiceIdx = 0; voiceIdx < MAX_CURRENT_SOUNDS; voiceIdx++)
    {
        xAudioVoice* voice = &voiceArr[voiceIdx];
        hr = xaudio2->CreateSourceVoice(&voice->voice, &wave, 
...
void platform_update_audio(float dt)
{
    for(int soundIdx = 0; soundIdx < soundState->playingSounds.count; soundIdx++)
    {
        ...
        SM_ASSERT(sound.size > 0, "Sound has no Sample Size: %d", sound.size);
